    ...
    extends UnsafeChunkEncoder
{
    ...
    private static int _getInt(final byte[] in, final int inPos) {
        return unsafe.getInt(in, BYTE_ARRAY_OFFSET + inPos);
    }

    private static int _getInt(final ByteBuffer in, final int inPos) {
        return in.getInt(inPos);
    }
    ...
    private static int _findMatchLength(final byte[] in, int ptr1, int ptr2, final int maxPtr1)
    {
    ...
    private static int _findMatchLength(final ByteBuffer in, int ptr1, int ptr2, final int maxPtr1)
    {
        // Expect at least 8 bytes to check for fast case; offline others
        if ((ptr1 + 8) >= maxPtr1) { // rare case, offline
            return _findTailMatchLength(in, ptr1, ptr2, maxPtr1);
    ...
    private static int _findLongMatchLength(final byte[] in, int ptr1, int ptr2, final int maxPtr1)
    {
    ...
    private static int _findLongMatchLength(final ByteBuffer in, int ptr1, int ptr2, final int maxPtr1)
    {
        final int base = ptr1 - 9;
        // and then just loop with longs if we get that far
        final int longEnd = maxPtr1-8;
    ...
    private static int _leadingBytes(int i1, int i2) {
        return Integer.numberOfLeadingZeros(i1 ^ i2) >> 3;
    }
    
    private static int _leadingBytes(long l1, long l2) {
